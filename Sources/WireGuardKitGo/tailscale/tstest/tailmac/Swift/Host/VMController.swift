...
class VMController: NSObject, VZVirtualMachineDelegate {
    ...
    func createVirtualMachine() {
        ...
        do {
            ...
            // Handle specific VM configuration errors
            print("VM Configuration failed: \(error.localizedDescription)")
    ...
    func restoreVirtualMachine() {
        virtualMachine.restoreMachineStateFrom(url: config.saveFileURL, completionHandler: { [self] (error) in
            ...
            do {
                ...
                // Fail gracefully if the file can't be removed
                print("Could not remove save file: \(error.localizedDescription)")
            }
            if error == nil {
